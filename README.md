...
needs to pick that up without a restart. Interval defaults to 1 hour (`CATALOGUE_REFRESH_SECONDS`, default 3600).
This is tuned via the default in each code drop rather than by env var
on the deployed side — no config change needed on the C# end. A failed
